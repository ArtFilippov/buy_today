...
from prak.schema import COLUMNS, DTYPES, ROW_KEY
...
def check_dataset(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    def check(name: str, passed: bool, actual: str | int, expected: str) -> None:
        ...
        records.append({
            "Проверка": name, "Результат": "OK",
            "Фактически": str(actual), "Ожидается": expected,
        })

    check(
        "Схема и порядок колонок", tuple(frame.columns) == COLUMNS,
        "35 колонок, порядок совпадает" if tuple(frame.columns) == COLUMNS else str(list(frame.columns)),
    ...
    check(
        "Типы и даты", not mismatched, str(mismatched) if mismatched else "35 типов совпадают",
        "типы schema.DTYPES, включая пять разобранных дат",
    )
    check("Непустой датасет", len(frame) > 0, len(frame), "число строк > 0")
    missing = int(frame.isna().sum().sum())
    check("Пропуски", missing == 0, missing, "число пропущенных значений = 0")
    duplicates = int(frame.duplicated(list(ROW_KEY)).sum())
    check(
        "Повторы ключа (order_id, order_item_id)", duplicates == 0,
        duplicates, "число повторений сверх первого = 0",
    )
    times = frame["order_purchase_timestamp"]
    inversions = int((times.diff() < pd.Timedelta(0)).sum())
    check(
        "Хронологический порядок покупок", inversions == 0,
        inversions, "число переходов назад во времени = 0",
    )
    invalid_prices = int((~np.isfinite(frame["price"]) | (frame["price"] <= 0)).sum())
    check(
        "Конечная положительная цена", invalid_prices == 0,
        invalid_prices, "число цен, нарушающих 0 < price < inf, = 0",
    )
...
def check_combination(reference: pd.DataFrame, batch: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    keys = pd.concat(
        [reference[list(ROW_KEY)], batch[list(ROW_KEY)]], ignore_index=True
    )
    ...
    return pd.DataFrame([{
        "Проверка": name, "Результат": "OK",
        "Фактически": str(duplicates), "Ожидается": expected,
    }])
