...
from prak.cli import _drift_threshold, _positive_float, _positive_int, _random_state
from prak.pipeline import PipelineConfig, run_next_batch
from prak.preparation import prepare_data
from prak.progress import stage
from prak.ranking import export_recommendations
from prak.summary import report_summary


WORKSPACE = Path("/workspace")
BUNDLED_DATA = Path("/opt/prak/olist")
# python -m executes this module as __main__; keep events in the configured tree.
_logger = logging.getLogger("prak.container_cli")
...
def _parser():
    parser = _ArgumentParser(
        prog="prak-container", description="Olist в Docker: init, update, inference; результаты в /workspace.",
    )
