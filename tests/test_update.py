...
def test_init_saves_exact_reference_before_reporting(
    working_frame, write_dataset, tmp_path, monkeypatch, existing,
):
    ...
    def report_saved(dataset_path, output_dir):
...
def test_invalid_init_leaves_old_reference(working_frame, write_dataset, tmp_path):
...
def test_failed_eda_keeps_newly_written_reference(working_frame, write_dataset, tmp_path, monkeypatch):
    reference = tmp_path / "reference.csv"
...
def test_update_reports_before_and_after_exact_sorted_merge(
    working_frame, new_batch, write_dataset, tmp_path, monkeypatch,
):
    ...
    new_batch["order_purchase_timestamp"] = (
        working_frame["order_purchase_timestamp"] + pd.Timedelta(minutes=30)
    )
    ...
    def report_before(batch_path, reference_path, output_dir, *, thresholds):
    ...
    def read_after_deda(path):
        assert calls == ["deda"]
        return read_dataset(path)

    def report_after(dataset_path, output_dir):
    ...
    assert batch.read_bytes() == batch_before
    assert reference.read_text(encoding="utf-8") == expected.to_csv(index=False, date_format=DATE_FORMAT)
    assert not reports.exists()
    assert not (tmp_path / "state.json").exists()


def test_update_missing_reference_fails_before_deda(tmp_path, monkeypatch):
...
def test_update_html_export_failure_preserves_reference(
    ...
    def fail_export(self, notebook, *args, **kwargs):
...
def test_update_real_deda_rejects_invalid_inputs_before_writing(
    working_frame, new_batch, write_dataset, tmp_path, monkeypatch, invalid, message,
):
    ...
    assert not (reports / "eda").exists()
    assert not (reports / "deda/report.html").exists()
    notebook = nbformat.read(reports / "deda/report.ipynb", as_version=4)
