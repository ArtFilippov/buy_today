...
import pandas as pd
import pytest
from fixture_types import DatasetWriter

from buy_today.auto_eda.checks import check_combination, check_dataset
from buy_today.schema import read_dataset


def test_complete_dataset_passes_with_repeated_products_and_tied_times(working_frame: pd.DataFrame):
...
def test_missing_values_fail(
    working_frame: pd.DataFrame, write_dataset: DatasetWriter, column: str
):
...
def test_invalid_prices_fail(
    working_frame: pd.DataFrame, write_dataset: DatasetWriter, price: float
):
...
def test_duplicate_key_reports_excess_count(working_frame: pd.DataFrame):
...
def test_unsorted_purchases_fail(working_frame: pd.DataFrame):
    with pytest.raises(ValueError, match="переходов назад во времени = 0"):
        check_dataset(working_frame.iloc[::-1])


def test_empty_csv_fails(working_frame: pd.DataFrame, write_dataset: DatasetWriter):
...
def test_strict_csv_columns(working_frame: pd.DataFrame, write_dataset: DatasetWriter, change: str):
    if change == "reorder":
...
@pytest.mark.parametrize(
    ("column", "value"),
    [
        ("order_item_id", "not-an-integer"),
        ("order_item_id", "1.5"),
        ("price", "not-a-price"),
        ("order_approved_at", "2018-02-30 00:00:00"),
    ],
)
def test_strict_csv_values(
    working_frame: pd.DataFrame, write_dataset: DatasetWriter, column: str, value: str
):
...
def test_dataframe_types_are_checked(working_frame: pd.DataFrame):
...
def test_combination_accepts_repeated_products_with_distinct_row_keys(
    working_frame: pd.DataFrame, new_orders: bool
):
    ...
    assert len(checks) == 1
    assert "объединении" in str(checks.at[0, "Проверка"])
...
@pytest.mark.parametrize(
    ("reference_rows", "batch_rows", "duplicates"),
    [
        ([0, 0, 0], [2, 3], 2),
        ([0, 1], [2, 2, 2, 2], 3),
        ([0, 0], [2, 2, 2], 3),
