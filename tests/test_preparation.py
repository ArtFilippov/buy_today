...
def test_sellers_counted_before_delivered_filter(raw_tables, write_raw, tmp_path):
...
def test_cleaning_and_untranslated_categories(raw_tables, write_raw, tmp_path):
...
def test_chronological_batches(raw_tables, write_raw, tmp_path, rows, batch_size, sizes, tail):
...
def test_positions_of_one_order_can_cross_batch_boundary(raw_tables, write_raw, tmp_path):
    items = raw_tables["olist_order_items_dataset.csv"]
...
def test_repreparation_resets_state_and_replaces_manifest(raw_tables, write_raw, tmp_path):
    ...
    assert [batch["path"] for batch in manifest["batches"]] == [
        "batches/batch_000.csv", "batches/batch_001.csv"
    ]
...
def test_invalid_batch_size_is_rejected_before_reading(tmp_path, batch_size):
    with pytest.raises(ValueError, match="positive integer"):
        prepare_data(tmp_path / "absent", tmp_path / "stream", batch_size)


@pytest.mark.parametrize("rows", [0, 1])
def test_too_few_rows(raw_tables, write_raw, tmp_path, rows):
    raw_tables["olist_order_items_dataset.csv"] = raw_tables["olist_order_items_dataset.csv"].iloc[:rows]
    with pytest.raises(ValueError, match="At least two complete rows"):
        prepare_data(write_raw(raw_tables), tmp_path / "stream")


def test_all_rows_removed_by_cleaning(raw_tables, write_raw, tmp_path):
...
def test_duplicate_keys_are_errors(raw_tables, write_raw, tmp_path, filename):
...
def test_missing_references_are_errors(raw_tables, write_raw, tmp_path, filename, column):
    if column == "seller_id":
...
def test_malformed_source_values_fail(raw_tables, write_raw, tmp_path, filename, column, value):
