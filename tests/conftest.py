...
import pandas as pd
import pytest
from fixture_types import CategoryTables, DatasetWriter, RawWriter
...
def working_frame() -> pd.DataFrame:
    """Build complete positions with tied times and distinct item keys.

    Returns:
        pd.DataFrame: Twelve positions with all working-schema columns.
    """
...
@pytest.fixture
def new_batch(working_frame: pd.DataFrame) -> pd.DataFrame:
    """Build a later, disjoint batch with repeated products.

    Args:
        working_frame (pd.DataFrame): Initial positions to copy.

    Returns:
        pd.DataFrame: Disjoint keys and timestamps shifted by one day.
    """
...
@pytest.fixture
def write_dataset(tmp_path: Path) -> DatasetWriter:
...
def raw_tables() -> dict[str, pd.DataFrame]:
    """Build small Olist sources with tied times and padded postal codes.

    Returns:
        dict[str, pd.DataFrame]: Seven raw tables indexed by CSV filename.
    """
...
@pytest.fixture
def raw_category_tables(raw_tables: dict[str, pd.DataFrame]) -> CategoryTables:
    """Build a source-table factory parameterized by category counts.

    Args:
        raw_tables (dict[str, pd.DataFrame]): Complete source templates.

    Returns:
        CategoryTables: Factory that preserves source keys and schema.
    """
    ...
    def build(category_counts: dict[str, int]) -> dict[str, pd.DataFrame]:
        ...
        product_ids = [
            f"product_{index}"
            for index, count in enumerate(category_counts.values())
        ...
        tables["olist_orders_dataset.csv"]["order_purchase_timestamp"] = pd.date_range(
            "2018-01-01",
            periods=len(order_ids),
            freq="h",
        ).strftime(DATE_FORMAT)
        tables["olist_products_dataset.csv"] = (
            tables["olist_products_dataset.csv"]
            .iloc[[0] * len(category_counts)]
            .reset_index(drop=True)
            .assign(
...
@pytest.fixture
def write_raw(tmp_path: Path) -> RawWriter:
