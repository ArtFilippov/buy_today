...
from container_support import (
    ...
    check_exports, file_hashes, read_json, seed_workspace, small_raw_tables, write_bundle,
)


@pytest.fixture
def container_environment(tmp_path, raw_category_tables, monkeypatch):
...
@pytest.fixture
def compact_reports(monkeypatch):
    ...
    def eda(dataset, output):
        ...
        return save(
            output, kind="eda", input=identity(dataset), rows=len(frame),
            n_orders=frame.order_id.nunique(), n_customers=frame.customer_id.nunique(),
            n_products=frame.product_id.nunique(), checks=check_dataset(frame).to_dict("records"),
        )

    def deda(batch, reference, output, *, thresholds):
        ...
        return save(
            output, kind="deda", inputs={
                "reference": {**identity(reference), "rows": len(old)},
                "batch": {**identity(batch), "rows": len(new)},
            }, thresholds=asdict(thresholds), metrics=metrics, drift_detected=result.drift_detected,
        )

    def clustering(dataset, batch, output, **kwargs):
        return save(
            output, kind="clustering", inputs={"dataset": identity(dataset), "new_batch": identity(batch)},
            silhouette=None, silhouette_reason="Notebook evaluation omitted in this unit test",
...
@pytest.fixture
def initialized(container_environment, compact_reports):
    workspace, bundle = container_environment
    container_cli.main(INIT_ARGS)
    return workspace, bundle


def invocation_log(workspace, command):
...
def test_init_resets_only_owned_outputs_and_overwrites_all_nine_raw_csvs(
    container_environment, compact_reports,
):
    workspace, bundle = container_environment
    ...
    assert [batch["rows"] for batch in read_json(workspace / "data/manifest.json")["batches"]] == [12, 12]
    assert all((workspace / name).is_file() for name in SUMMARY_FILES)
    assert len(list((workspace / "logs").glob("*.log"))) == 2


@pytest.mark.parametrize("filename,problem", [
...
def test_invalid_bundle_is_rejected_before_any_reset_or_raw_overwrite(
    container_environment, monkeypatch, filename, problem,
):
    workspace, bundle = container_environment
    seed_workspace(workspace)
...
def test_bad_config_preserves_existing_experiment_before_reset(
    container_environment, monkeypatch, options, error,
):
...
def test_argument_failure_has_a_persistent_diagnostic(container_environment, arguments, diagnostic):
    workspace, _ = container_environment
...
def test_help_does_not_require_a_writable_workspace(container_environment):
    workspace, _ = container_environment
    with pytest.raises(SystemExit) as caught:
...
def test_symlink_destination_is_rejected_before_deleting_any_existing_outputs(
    container_environment, tmp_path, name,
):
    workspace, _ = container_environment
    seed_workspace(workspace)
...
def test_update_resumes_frozen_config_without_export_or_preparation(initialized, monkeypatch):
    ...
    pd.testing.assert_frame_equal(
        second.events.loc[second.events.batch_index.eq(0)].reset_index(drop=True), first_history.events,
    )
    assert len(second.anchors) == 9 and len(second.catalog) == 6
    assert [step["step_index"] for step in read_json(run / "summary/summary.json")["steps"]] == [0, 1]
...
def test_csv_destination_directory_is_rejected_before_reset(container_environment):
    workspace, _ = container_environment
    seed_workspace(workspace)
...
def test_inference_uses_explicit_saved_model_and_unique_default_output_without_histories(
    ...
    for index, user in enumerate(("user_000000_000000", "user_000001_000000")):
        ...
        selected = relative if index == 0 else workspace / relative
        container_cli.main(["inference", "--model-dir", str(selected), "--user-id", user, "--k", "2"])
        output, = set((workspace / "recommendations").glob("*.csv")) - existing
        table = pd.read_csv(output)
        expected = pd.DataFrame({"user_id": [user] * 2, "rank": [1, 2], "product_id": model.predict(user, 2)})
    ...
    with pytest.raises(ValueError, match="Unknown user_id"):
        container_cli.main([
            "inference", "--model-dir", "run/steps/step_000/ranking",
            "--user-id", "user_000001_000000", "--k", "2",
        ])
...
def test_inference_cannot_write_outside_recommendations(container_environment, monkeypatch, destination):
    workspace, _ = container_environment
    seed_workspace(workspace)
    ...
    with pytest.raises(ValueError, match="inside /workspace/recommendations"):
        container_cli.main([
            "inference", "--model-dir", "run/steps/step_000/ranking", "--user-id", "known",
            "--output", destination,
        ])
...
def test_inference_does_not_overwrite_run_file_through_hardlink(container_environment, monkeypatch):
    workspace, _ = container_environment
    seed_workspace(workspace)
    victim = workspace / "run/config.json"
    victim.write_bytes(b"existing run configuration")
...
def test_summary_failure_keeps_committed_step_logs_progress_and_reraises_original(
    container_environment, compact_reports, monkeypatch,
):
    workspace, _ = container_environment
    error = OSError("summary destination full")

    def fail_summary(run):
    ...
    first = file_hashes(run / "steps/step_000")
    log = invocation_log(workspace, "init")
    assert log.index("event=batch_committed batch_index=0") < log.index("stage=summary event=failed")
...
def test_exhausted_update_rebuilds_summary_without_changing_training_state(initialized, monkeypatch, capsys):
    workspace, _ = initialized
    container_cli.main(["update"])
...
def test_stages_are_live_when_verbose_and_always_in_file_log(
    container_environment, monkeypatch, capsys, verbose,
):
    workspace, _ = container_environment
    monkeypatch.setattr(container_cli, "run_next_batch", lambda run: None)
    observed = []
    ...
    def summarize(run):
        ...
        with stage("summary_details"):
            pass
        return SummaryPaths(*(run / f"summary/summary.{suffix}" for suffix in ("json", "csv", "html")))
...
def test_handlers_levels_and_propagation_restored_after_success_failure_and_repeated_calls(
    ...
    for index in range(3):
        ...
        assert all(handler.stream is None for handler in active_handlers if isinstance(handler, logging.FileHandler))
...
def test_update_requires_initialized_run_without_exporting_or_preparing(container_environment, monkeypatch):
    workspace, _ = container_environment
    monkeypatch.setattr(container_cli, "prepare_data", forbidden)
    monkeypatch.setattr(container_cli, "_reset_and_export", forbidden)
...
def test_entrypoint_module_name_does_not_change_command_logging(container_environment, monkeypatch):
    workspace, _ = container_environment
