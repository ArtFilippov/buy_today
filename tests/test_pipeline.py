...
from buy_today.pipeline import (
    PipelineConfig,
...
@pytest.fixture
def stream(tmp_path: Path, working_frame: ft.DataFrame, new_batch: ft.DataFrame) -> Path:
    root = tmp_path / "prepared stream"
    (root / "batches").mkdir(parents=True)
    batches: list[dict[str, str | int]] = []
    for index, original in enumerate((working_frame, new_batch)):
        frame = original.copy()
...
@pytest.fixture
def fast_reports(monkeypatch: ft.MonkeyPatch) -> None:
    """Skip kernel startup only; real references, histories and rankers still run.

    Args:
        monkeypatch (ft.MonkeyPatch): Fixture replacing the report exporters.
    """

    def save(output: Path | str, payload: dict[str, object]) -> ReportPaths:
        output = Path(output)
    ...
    def deda(
        batch: Path | str, reference: Path | str, output: Path | str, **kwargs: object
    ) -> ReportPaths:
        check_combination(read_dataset(reference), read_dataset(batch))
        return save(output, {"kind": "deda"})

    def clustering(
        dataset: Path | str, batch: Path | str, output: Path | str, **kwargs: object
    ) -> ReportPaths:
...
def test_two_steps_real_training_preserves_old_histories_and_owns_progress(
    ...
    remaining = run_pipeline(run, all_batches=True)
    ...
    old_events = second_history.events[second_history.events.batch_index.eq(0)].reset_index(
        drop=True
    )
    ...
    assert not run_pipeline(run, all_batches=True)
    assert (run / "state.json").read_bytes() == state_before
    assert read_pipeline_config(run) == replace(small_config, model=model)


def test_full_two_batch_notebook_pipeline_summary_and_inference(
    tmp_path: Path, stream: Path, small_config: PipelineConfig
) -> None:
...
def test_changed_inputs_are_rejected_before_starting_next_step(
    ...
    paths = {
        ...
        "batch": stream / "batches/batch_001.csv",
...
def test_valid_rebuilt_history_snapshot_cannot_replace_committed_lineage(
    ...
    generate_dataset(
        stream / "batches/batch_000.csv",
        first.output_dir / "clustering/distance.joblib",
        alternate,
        temperature=small_config.temperature,
        n_users=small_config.initial_users,
        split_sizes=small_config.split_sizes,
        random_state=7,
    )
...
def test_frozen_run_parameters_and_data_source(
    tmp_path: Path, stream: Path, small_config: PipelineConfig, fast_reports: None
) -> None:
    run = tmp_path / "run"
    run_next_batch(run, data_dir=stream, config=small_config)
...
def test_new_run_requires_stream_and_rejects_existing_directories(
    tmp_path: Path, stream: Path, small_config: PipelineConfig
) -> None:
    run = tmp_path / "run"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"svd_n_components": 0},
        {"svd_n_iter": -1},
        {"temporal_n_clusters": 0},
        {"max_evaluation_rows": False},
        {"random_state": -1},
        {"thresholds": {}},
    ],
)
def test_invalid_run_settings(options: dict[str, Any]) -> None:
    with pytest.raises(ValueError):
        PipelineConfig(**options)


def test_two_fresh_runs_reproduce_histories_models_and_scores(
    tmp_path: Path, stream: Path, small_config: PipelineConfig, fast_reports: None
) -> None:
    ...
    for left, right in zip(first, second, strict=True):
        for relative in (
            "histories/train.csv",
            "histories/validation.csv",
            "histories/test.csv",
            "histories/catalog.csv",
            "ranking/model.joblib",
            "clustering/labels.csv",
        ):
            assert (left.output_dir / relative).read_bytes() == (
                right.output_dir / relative
            ).read_bytes()
        for split in ("validation", "test"):
...
def test_inconsistent_progress_is_rejected(
    tmp_path: Path, stream: Path, small_config: PipelineConfig, fast_reports: None
) -> None:
    run = tmp_path / "run"
    run_next_batch(run, data_dir=stream, config=small_config)
...
def test_cli_run_configuration_continuation_and_exhaustion(
    tmp_path: Path, stream: Path, fast_reports: None, capsys: ft.CaptureFixture[str]
) -> None:
    ...
    assert read_pipeline_config(run).thresholds.price == 0.2
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            ["--model-dir", "unused", "--user-id", "u", "--output", "unused.csv"],
        ),
        ("summary", "report_summary", ["--run-dir", "unused"]),
    ],
)
def test_new_cli_commands_propagate_original_exceptions(
    monkeypatch: ft.MonkeyPatch, command: str, function: str, args: list[str]
) -> None:
...
def test_new_cli_help(command: str, capsys: ft.CaptureFixture[str]) -> None:
...
def test_cli_rejects_svd_options_for_random(tmp_path: Path) -> None:
    with pytest.raises(SystemExit) as caught:
        main(
            [
                "run",
