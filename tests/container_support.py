...
def small_raw_tables(raw_category_tables: ft.CategoryTables) -> ft.RawTables:
    """Build 24 positions: two batches with three old and three new products.

    Args:
        raw_category_tables (ft.CategoryTables): Factory for the raw Olist tables.

    Returns:
        ft.RawTables: Nine source tables, including payment and review fixtures.
    """
    ...
    tables["olist_order_reviews_dataset.csv"] = pd.DataFrame(
        {
            ...
            "review_comment_message": "Delivered",
...
def write_bundle(directory: Path, tables: ft.RawTables) -> Path:
...
def seed_reset_targets(workspace: Path) -> None:
...
def assert_preserved(directory: Path, expected: Mapping[str, str]) -> None:
    actual = file_hashes(directory)
    for name, digest in expected.items():
        assert actual.get(name) == digest, f"Changed or missing file: {directory / name}"


def check_exports(workspace: Path, bundle: Path) -> None:
...
def check_reports(workspace: Path, count: int) -> None:
    run = workspace / "run"
    ...
    assert [step["data_quality"]["rows"] for step in summary["steps"]] == [
        12 * (i + 1) for i in range(count)
    ]
    assert [step["ranking"]["test"]["n_users"] for step in summary["steps"]] == [
        6 + 3 * i for i in range(count)
    ]
...
def check_log(
    ...
    candidates = [
        path for path in (workspace / "logs").glob(f"*_{command}.log") if path.name not in previous
    ]
...
def create(root: Path) -> None:
    ...
    fixtures = import_module("conftest")
    make_tables = cast(Callable[[], ft.RawTables], unwrap(fixtures.raw_tables))
    make_categories = cast(
        Callable[[ft.RawTables], ft.CategoryTables], unwrap(fixtures.raw_category_tables)
    )
    factory = make_categories(make_tables())
...
def snapshot(root: Path, workspace: Path, name: str) -> None:
...
def check(root: Path, workspace: Path, phase: str) -> None:
    ...
    if phase in {"init", "reinit"}:
        ...
        previous: dict[str, dict[str, str]] = (
            read_json(root / "snapshots/reinit.json") if phase == "reinit" else {"logs": {}}
        )
        ...
        if phase == "update":
            protected = {
                key: value
                for key, value in previous["workspace"].items()
                if key.startswith(("run/steps/step_000/", "data/", "dataset/"))
                or key == "run/config.json"
            }
            assert_preserved(workspace, protected)
            text = check_log(
                workspace, "update", stages + training_stages + ["reference.deda"], previous["logs"]
            )
            ...
            check_log(
                workspace,
                "inference",
                ["inference", "inference.load", "inference.predict", "inference.export"],
            )
        else:
            ...
            current = {
                key: value
                for key, value in file_hashes(workspace).items()
...
def main() -> None:
    ...
    print(
        f"PASS: {args.action} {args.phase or args.name or 'synthetic nine-table bundle'}",
        flush=True,
    )
