...
from buy_today.generation.records import HistoryDataset
from buy_today.schema import ROW_KEY


class AnchorOnlyDistance:
    def pairwise(self, left: pd.DataFrame, right: pd.DataFrame) -> NDArray[np.float64]:
        same_key = (right.order_id == left.order_id.iloc[0]) & (
            right.order_item_id == left.order_item_id.iloc[0]
        )
...
@pytest.fixture
def distance_path(tmp_path: ft.Path) -> ft.Path:
...
def test_two_steps_roundtrip_preserve_old_histories_with_changed_distance(
    ...
    assert set(old_files) == {
        ...
        "generator/sources.csv",
        "generator/anchors.csv",
        "generator/distances/batch_000.joblib",
    }
    ...
    pd.testing.assert_frame_equal(
        accumulated.events[accumulated.events.batch_index.eq(0)], saved.events
    )
    ...
    assert set(accumulated.catalog.product_id) == set(working_frame.product_id) | set(
        new_batch.product_id
    )
    assert_anchor_only_histories(accumulated)
    assert_manifest_provenance(
        accumulated, later.output_dir, (batch0, batch1), (distance_path, changed_distance),
    )

    # Repeating the same two-step experiment preserves every serialized file.
    copy0 = generate_dataset(
        batch0,
        distance_path,
        tmp_path / "copy0",
        temperature=86400,
    ...
    copy1 = generate_dataset(
        batch1,
        changed_distance,
        tmp_path / "copy1",
        previous_dir=copy0.output_dir,
        temperature=1,
        n_users=2,
    )
...
def assert_anchor_only_histories(data: HistoryDataset) -> None:
    for user, events in data.events[data.events.batch_index == 1].groupby("user_id"):
        anchor = data.anchors.set_index("user_id").loc[user]
...
def assert_manifest_provenance(
    data: HistoryDataset, directory: ft.Path, sources: tuple[ft.Path, ft.Path],
    distances: tuple[ft.Path, ft.Path],
) -> None:
    manifest = data.manifest
    assert manifest["format_version"] == 1
    assert manifest["split_sizes"] == [3, 2, 1]
    assert [record["n_users"] for record in manifest["batches"]] == [3, 2]
    for record, source, distance in zip(manifest["batches"], sources, distances):
        ...
        assert (directory / record["distance"]["snapshot"]).read_bytes() == distance.read_bytes()
        assert record["random_state"] == 42


def test_default_user_counts_and_catalog_includes_unsampled_products(
    working_frame: ft.DataFrame, new_batch: ft.DataFrame, write_dataset: ft.DatasetWriter,
    distance_path: ft.Path, tmp_path: ft.Path,
) -> None:
    batch0, batch1 = write_dataset(working_frame, "zero.csv"), write_dataset(new_batch, "one.csv")
    first = generate_dataset(
        batch0, distance_path, tmp_path / "step0", temperature=86400, split_sizes=(1, 1, 1)
    )
    second = generate_dataset(
        batch1, distance_path, tmp_path / "step1", temperature=86400, previous_dir=first.output_dir
    )
    data = read_history_dataset(second.output_dir)
    assert data.anchors.groupby("batch_index").size().to_dict() == {0: 2000, 1: 250}
    assert data.events.groupby("split").size().to_dict() == {
        "train": 2250,
        "validation": 2250,
        "test": 2250,
    }
...
def test_continuation_does_not_require_original_files(
    working_frame: ft.DataFrame, new_batch: ft.DataFrame, write_dataset: ft.DatasetWriter,
    distance_path: ft.Path, tmp_path: ft.Path,
) -> None:
...
def test_repeat_or_overlap_even_unsampled_source_is_rejected(
    working_frame: ft.DataFrame, write_dataset: ft.DatasetWriter, distance_path: ft.Path,
    tmp_path: ft.Path, partial: bool,
) -> None:
    first = generate_dataset(
        write_dataset(working_frame),
        distance_path,
    ...
    with pytest.raises(ValueError, match="overlaps"):
        generate_dataset(
            write_dataset(frame, "renamed.csv"),
            distance_path,
            tmp_path / "step1",
            previous_dir=first.output_dir,
            temperature=86400,
...
def test_cannot_change_splits_or_write_into_previous_snapshot(
    working_frame: ft.DataFrame, new_batch: ft.DataFrame, write_dataset: ft.DatasetWriter,
    distance_path: ft.Path, tmp_path: ft.Path,
) -> None:
    first = generate_dataset(
...
def test_corrupted_snapshot_fails_before_new_sampling(
    working_frame: ft.DataFrame, new_batch: ft.DataFrame, write_dataset: ft.DatasetWriter,
    distance_path: ft.Path, tmp_path: ft.Path,
) -> None:
    first = generate_dataset(
    ...
    first.train_path.write_text(
        first.train_path.read_text(encoding="utf-8") + "corruption\n", encoding="utf-8",
    )
...
@pytest.mark.parametrize(
    "problem", ["split", "product", "source_key", "duplicate_event", "catalog", "anchor"]
)
def test_snapshot_semantics_checked_even_with_updated_checksums(
    working_frame: ft.DataFrame, write_dataset: ft.DatasetWriter, distance_path: ft.Path,
    tmp_path: ft.Path, problem: str,
) -> None:
...
def test_failure_does_not_publish_next_snapshot(
    working_frame: ft.DataFrame, new_batch: ft.DataFrame, write_dataset: ft.DatasetWriter,
    distance_path: ft.Path, tmp_path: ft.Path, monkeypatch: ft.MonkeyPatch, failure: str,
) -> None:
    first = generate_dataset(
        write_dataset(working_frame),
        distance_path,
    ...
    if failure == "distance":
        ...
        def failed_write(
            self: pd.DataFrame, path: ft.Path, *, index: bool, encoding: str, date_format: str,
        ) -> None:
            calls.append(path)
            if len(calls) == 2:
                raise OSError("write failed")
            original(self, path, index=index, encoding=encoding, date_format=date_format)
