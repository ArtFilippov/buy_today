...
def test_generate_two_steps_from_another_cwd(
    ...
    for index, batch in enumerate((batch0, batch1)):
        args = [
            str(Path(sys.executable).with_name("buy_today")),
            "generate",
            "--batch",
            batch.name,
            "--distance",
        ...
        args += ["--split-sizes", "3", "2", "1"] if not index else ["--previous-dir", "step 0"]
        result = subprocess.run(args, cwd=tmp_path, text=True, capture_output=True, check=False)
...
def test_invalid_generate_arguments_exit_two(args: list[str]) -> None:
...
def test_generate_requires_paths(missing: str) -> None:
...
def test_generation_data_error_exits_one(
