...
from buy_today.generation.records import Integer, PairwiseDistance, SplitSizes
...
class ReadyDistance:
    ...
    Args:
        batch (pd.DataFrame): Source rows identifying the matrix axes.
        values (ArrayLike): Ready rectangular distances addressed by source keys.
    """

    def __init__(self, batch: pd.DataFrame, values: ArrayLike) -> None:
        super().__init__()
        self.keys = pd.MultiIndex.from_frame(batch[list(ROW_KEY)])
        self.values = np.asarray(values)
        self.calls: list[tuple[pd.DataFrame, pd.DataFrame]] = []

    def pairwise(self, left_frame: pd.DataFrame, right_frame: pd.DataFrame) -> ArrayLike:
        self.calls.append((left_frame.copy(deep=True), right_frame.copy(deep=True)))
        left = self.keys.get_indexer(pd.MultiIndex.from_frame(left_frame[list(ROW_KEY)]))
        right = self.keys.get_indexer(pd.MultiIndex.from_frame(right_frame[list(ROW_KEY)]))
...
class DrawOptions(TypedDict):
    n_users: Integer
    batch_index: Integer
    random_state: NotRequired[Integer]
    split_sizes: SplitSizes


class DistanceOptions(TypedDict):
...
def reference_draws(
    probabilities: NDArray[np.float64], *, n_users: Integer, batch_index: Integer,
    random_state: Integer = 42, split_sizes: SplitSizes = (70, 15, 15),
) -> tuple[NDArray[np.int64], NDArray[np.int64]]:
    """Apply the specified RNG protocol with independently supplied probabilities.

    Args:
        probabilities (NDArray[np.float64]): Row-wise source probabilities per anchor.
        n_users (Integer): Number of synthetic users.
        batch_index (Integer): Batch number in the seed sequence.
        random_state (Integer, default=42): Seed for deterministic generation.
        split_sizes (SplitSizes, default=(70, 15, 15)): Train, validation and test history lengths.

    Returns:
        tuple[NDArray[np.int64], NDArray[np.int64]]: Anchor and history source positions.
    """
    rng = np.random.default_rng(np.random.SeedSequence([int(random_state), int(batch_index)]))
    anchors = rng.integers(len(probabilities), size=n_users)
    histories = np.array(
        [
            rng.choice(
                len(probabilities), size=sum(split_sizes), replace=True, p=probabilities[anchor]
            )
...
def assert_draws(
    result: GeneratedHistories, batch: pd.DataFrame, anchor_positions: NDArray[np.int64],
    history_positions: NDArray[np.int64],
) -> None:
    ...
    np.testing.assert_array_equal(
        result.events["user_id"],
        np.repeat(result.anchors["user_id"], history_positions.shape[1]),
    )


def test_default_schema_weighted_sampling_and_ready_rectangular_distance(
    sampling_batch: pd.DataFrame,
) -> None:
    # exp(-log(2) * k) gives exact, independently known relative weights 2**-k.
    distance = ReadyDistance(
        sampling_batch,
        np.log(2)
        * np.array(
            [
    ...
    result = generate_histories(
        sampling_batch, distance=distance, batch_index=7, n_users=6, temperature=1
    )
    ...
    assert (
        result.events["split"].tolist()
        == (["train"] * 70 + ["validation"] * 15 + ["test"] * 15) * 6
    )
    # Sampling order is synthetic time, even when the next source purchase is older.
    assert (
        result.events.groupby("user_id")["order_purchase_timestamp"].diff() < pd.Timedelta(0)
    ).any()
...
def test_temperature_is_in_seconds_and_custom_splits_follow_draw_order(
    ...
    distance = (
        TimestampDistance()
        if use_timestamp_distance
        else ReadyDistance(sampling_batch, days * 86400)
    )
    weights = np.exp(-days / (temperature / 86400))
    args: DrawOptions = {
        "n_users": 4, "batch_index": 2, "random_state": 13, "split_sizes": (7, 3, 2),
    }
    ...
    assert (
        result.events["split"].tolist() == (["train"] * 7 + ["validation"] * 3 + ["test"] * 2) * 4
    )


@pytest.mark.parametrize("n_rows", [1, 3])
def test_zero_distances_sample_rows_with_replacement_and_keep_repeated_products(
    sampling_batch: pd.DataFrame, n_rows: int
) -> None:
    batch = sampling_batch.iloc[:n_rows].copy()
    args: DrawOptions = {"n_users": 6, "batch_index": 0, "split_sizes": (4, 2, 2)}
    anchors, histories = reference_draws(np.full((n_rows, n_rows), 1 / n_rows), **args)
    result = generate_histories(
        batch,
        distance=ReadyDistance(batch, np.zeros((n_rows, n_rows))),
        temperature=1,
        **args,
    )
...
@pytest.mark.parametrize(
    "change", [{"random_state": 43}, {"batch_index": 1}], ids=["seed", "batch"]
)
def test_reproducibility_and_independent_seed_sequence_streams(
    sampling_batch: pd.DataFrame, change: dict[str, int],
) -> None:
    distance = ReadyDistance(sampling_batch, np.zeros((4, 4)))
    args: DrawOptions = {
        "n_users": 7, "batch_index": 0, "random_state": 42, "split_sizes": (4, 2, 1),
    }
    first = generate_histories(sampling_batch, distance=distance, temperature=1, **args)
    changed_args: DrawOptions = {
        **args, "random_state": change.get("random_state", 42),
        "batch_index": change.get("batch_index", 0),
    }
...
def test_next_batch_has_new_ids_and_only_its_own_sources(
    working_frame: ft.DataFrame, new_batch: ft.DataFrame,
) -> None:
    args: DistanceOptions = {
        "distance": TimestampDistance(), "temperature": 86400, "split_sizes": (3, 1, 1),
    }
...
@pytest.mark.parametrize(
    ("distances", "temperature", "probabilities"),
    [
        ...
        pytest.param(
            ...
            np.finfo(float).tiny,
            [0, 0.5, 0.5, 0],
            id="large-tied-minima",
        ),
        pytest.param(
            [0, 1, 2, 3], np.nextafter(0.0, 1.0), [1, 0, 0, 0], id="subnormal-temperature"
        ),
        pytest.param([0, 1, 2, 3], float_info.max, [0.25] * 4, id="huge-temperature"),
        pytest.param(
            [1e308, 0, 1e308, 0],
            1e308,
...
def test_extreme_temperatures_have_stable_normalized_sampling(
    sampling_batch: pd.DataFrame, distances: ArrayLike, temperature: float, probabilities: ArrayLike
) -> None:
    args: DrawOptions = {"n_users": 3, "batch_index": 0, "split_sizes": (4, 2, 2)}
    anchors, histories = reference_draws(np.tile(probabilities, (4, 1)), **args)
    with np.errstate(over="raise", invalid="raise", divide="raise"):
        result = generate_histories(
            sampling_batch,
            distance=ReadyDistance(sampling_batch, np.tile(distances, (4, 1))),
...
def test_numpy_integer_parameters_and_list_split_sizes_are_accepted(
    sampling_batch: pd.DataFrame, random_state: int,
) -> None:
    args: DrawOptions = {
        "n_users": np.int64(3), "batch_index": np.int64(2),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("n_users", "2"),
        ("n_users", True),
        ("n_users", np.bool_(True)),
        ("n_users", None),
        ("batch_index", -1),
        ("batch_index", 1.5),
        ("batch_index", "0"),
        ("batch_index", True),
        ("batch_index", np.bool_(False)),
        ("batch_index", None),
        ("random_state", -1),
        ("random_state", 2**32),
        ("random_state", 42.0),
        ("random_state", True),
        ("random_state", np.bool_(True)),
        ("random_state", None),
        ("temperature", 0),
        ("temperature", -1),
        ("temperature", np.nan),
        ...
        ("split_sizes", (1, 1.5, 1)),
        ("split_sizes", (1, 1, "1")),
        ("split_sizes", (True, 1, 1)),
        ("split_sizes", (1, np.bool_(True), 1)),
    ],
)
def test_invalid_parameters_fail_before_pairwise(
    sampling_batch: pd.DataFrame, parameter: str, value: Any,
) -> None:
    distance = ReadyDistance(sampling_batch, np.zeros((4, 4)))
    args: dict[str, Any] = {
        "n_users": 2, "batch_index": 0, "temperature": 1, "random_state": 42,
        "split_sizes": (2, 1, 1),
    }
    args[parameter] = value
    before = sampling_batch.copy(deep=True)
    with pytest.raises(
        ValueError, match="split_sizes|size" if parameter == "split_sizes" else parameter
    ):
        generate_histories(sampling_batch, distance=distance, **args)
    assert not distance.calls
    pd.testing.assert_frame_equal(sampling_batch, before)
...
def test_malformed_distances_fail_on_first_anchor(
    sampling_batch: pd.DataFrame, matrix: ArrayLike,
) -> None:
    class MalformedDistance:
        ...
        def pairwise(self, left: pd.DataFrame, right: pd.DataFrame) -> ArrayLike:
            self.calls += 1
            assert left.shape == (1, sampling_batch.shape[1])
            pd.testing.assert_frame_equal(right, sampling_batch)
            return matrix
...
def malformed_batch(batch: pd.DataFrame, problem: str) -> pd.DataFrame:
    transforms: dict[str, Callable[[pd.DataFrame], pd.DataFrame]] = {
        "missing_column": lambda frame: frame.drop(columns="customer_city"),
        "column_order": lambda frame: frame[frame.columns[::-1]],
        "empty": lambda frame: frame.iloc[:0],
        "unsorted": lambda frame: frame.iloc[::-1],
    }
    if problem in transforms:
        return transforms[problem](batch)
    if problem == "extra_column":
        ...
        batch.iloc[1, list(batch.columns).index("order_item_id")] = 1
    return batch
...
@pytest.mark.parametrize(
    ("problem", "message"),
    [
        ("missing_column", "Схема"),
        ("extra_column", "Схема"),
        ("column_order", "Схема"),
        ("item_dtype", "Типы и даты"),
        ("text_timestamp", "Типы и даты"),
        ("empty", "Непустой датасет"),
        ("missing_product", "Пропуски"),
        ("missing_timestamp", "Пропуски"),
        ("missing_customer", "Пропуски"),
        ("duplicate_key", "Повторы ключа"),
        ("unsorted", "Хронологический порядок"),
    ],
)
def test_invalid_olist_data_fail_before_pairwise(
    sampling_batch: pd.DataFrame, problem: str, message: str,
) -> None:
    distance = ReadyDistance(sampling_batch, np.zeros((4, 4)))
    batch = malformed_batch(sampling_batch.copy(), problem)
...
def test_invalid_olist_prices_fail_before_pairwise(
    sampling_batch: pd.DataFrame, price: float,
) -> None:
    sampling_batch.loc[-5, "price"] = price
    distance = ReadyDistance(sampling_batch, np.zeros((4, 4)))
    with pytest.raises(ValueError, match="Пропуски|Конечная положительная цена"):
        generate_histories(
            sampling_batch, distance=distance, batch_index=0, n_users=2, temperature=1
        )
    assert not distance.calls
