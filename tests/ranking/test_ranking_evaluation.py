...
class PredictOnlyRanker:
    ...
    Args:
        predictions (dict[str, Any]): Deliberately valid or invalid prediction responses.

    """

    def __init__(self, predictions: dict[str, Any]) -> None:
        super().__init__()
        self.predictions = predictions
        self.calls: list[tuple[str, int]] = []

    def predict(self, user_id: str, k: int, /) -> Any:
        self.calls.append((user_id, k))
        return self.predictions[user_id]


@pytest.fixture
def held_out() -> RankingData:
    return RankingData(
...
def test_binary_metrics_and_macro_average_use_only_predict():
    ...
    pd.testing.assert_frame_equal(
        result.per_user.set_index("user_id").sort_index(),
        expected,
        check_exact=False,
        rtol=1e-12,
        atol=1e-12,
...
def test_no_hit_and_perfect_hit_metrics(
    held_out: RankingData, recommendations: list[str], score: float, hits: int
):
...
def test_invalid_predictions_are_rejected(held_out: RankingData, prediction: Any, message: str):
...
def test_invalid_k_is_rejected_before_predicting(held_out: RankingData, k: object):
...
def test_empty_holdout_is_rejected_before_predicting(held_out: RankingData):
    model = PredictOnlyRanker({})
