...
def purchases(counts: list[list[int]] | NDArray[np.int64]) -> RankingData:
    return RankingData(
        pd.DataFrame(
            [
                (f"u{user}", f"p{item}")
                for user, row in enumerate(counts)
                for item, count in enumerate(row)
...
def test_truncated_scores_and_rankings_match_dense_svd(training_data: RankingData):
...
def test_exact_score_ties_negative_scores_and_k_boundary(training_data: RankingData):
    model = SVDRanker(n_components=2).fit(training_data)
...
def test_small_and_full_dimension_matrices(
    counts: list[list[int]] | NDArray[np.int64], n_components: int
):
    model = SVDRanker(n_components=n_components).fit(purchases(counts))
    actual = model.user_factors_ @ model.item_factors_.T
    np.testing.assert_allclose(actual, counts, atol=2e-5)
    cold = np.equal(np.asarray(counts).sum(axis=0), 0)
...
def test_sklearn_parameters_clone_and_refit_replace_state(training_data: RankingData):
    model = SVDRanker()
...
def test_row_and_call_order_reproducibility_and_input_isolation(training_data: RankingData):
    ...
    shuffled = SVDRanker(n_components=2).fit(
        RankingData(
...
def test_invalid_parameters_fail_on_fit(training_data: RankingData, parameter: str, value: object):
...
def test_invalid_k(training_data: RankingData, k: object):
    model = SVDRanker(n_components=2).fit(training_data)
...
def test_unknown_or_nonstring_user(training_data: RankingData, user: object):
    model = SVDRanker(n_components=2).fit(training_data)
    with pytest.raises(ValueError, match="Unknown user"):
        model.predict(user, 1)


def test_fit_validates_ranking_data(training_data: RankingData):
    incomplete: Any = training_data.interactions
    with pytest.raises(ValueError, match="RankingData"):
        SVDRanker(n_components=2).fit(incomplete)
    with pytest.raises(ValueError, match="outside catalog"):
        SVDRanker(n_components=2).fit(
            RankingData(
                training_data.interactions,
                training_data.catalog.iloc[1:],
            )
        )


def test_persistence_and_fresh_fit_across_process_hash_seeds(
