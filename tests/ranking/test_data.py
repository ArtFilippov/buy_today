...
def write_snapshot(directory, events, catalog, split="train"):
    events.to_csv(directory / f"{split}.csv", index=False)
    catalog.to_csv(directory / "catalog.csv", index=False)


def test_ranking_data_retains_repeated_purchases_and_unobserved_candidates(ranking_data):
...
@pytest.mark.parametrize(("table", "problem", "message"), [
...
def test_ranking_data_rejects_invalid_tables(ranking_data, table, problem, message):
    frame = getattr(ranking_data, table)
    if problem == "missing_column":
        ...
        frame = frame.iloc[1:]
    tables = {"interactions": ranking_data.interactions, "catalog": ranking_data.catalog}
    tables[table] = frame
...
def test_identifiers_must_be_present_nonempty_strings(ranking_data, table, column, value):
    tables = {
...
def test_plain_interactions_are_not_a_complete_ranking_input(ranking_data):
...
def test_reader_opens_only_selected_split_and_catalog_and_preserves_ids(
    tmp_path, monkeypatch, ranking_data, events, split,
):
    # Other splits and all generator state are absent. Provenance is not a model input.
    events = events.assign(
        split=split, order_purchase_timestamp="not a timestamp", cluster_id=99,
...
def test_reader_rejects_unknown_split_before_opening_files(tmp_path):
...
def test_reader_rejects_invalid_events(
    tmp_path, ranking_data, events, column, row, value, message,
):
...
@pytest.mark.parametrize(("products", "message"), [
    (["001", "002", "NA", "001"], "Duplicate"),
    (["001", "002", "unsampled"], "outside catalog"),
])
def test_reader_requires_unique_catalog_covering_every_purchase(tmp_path, events, products, message):
