...
def test_training_reads_only_train_and_catalog_and_records_provenance(
    ...
    fitted: list[ft.RankingData] = []
    original_random_fit, original_svd_fit = RandomRanker.fit, SVDRanker.fit

    def record_fit(self: Ranker, data: ft.RankingData, y: object = None) -> Ranker:
        fitted.append(data)
        if isinstance(self, RandomRanker):
            return original_random_fit(self, data, y)
        return original_svd_fit(self, data, y)
...
@pytest.mark.parametrize(
    "ranker", [RandomRanker(), SVDRanker(n_components=1)], ids=["random", "svd"]
)
def test_saved_evaluation_needs_only_heldout_and_catalog_and_never_fits(
    ...
    assert (
        metrics["inputs"][split]["sha256"]
        == hashlib.sha256(before_data[f"{split}.csv"]).hexdigest()
    )
...
def test_incompatible_evaluation_fails_without_report(
    ranking_snapshot: ft.Path, tmp_path: ft.Path, problem: str
):
    model_dir, output = tmp_path / "model", tmp_path / "report"
    paths = train_ranker(ranking_snapshot, model_dir)
    split, k = "validation", 10
    if problem == "catalog":
        with (ranking_snapshot / "catalog.csv").open("a", encoding="utf-8") as file:
            file.write("other\n")
        ...
        paths.manifest_path.write_text('{"format_version": 99}', encoding="utf-8")
...
def test_destinations_protect_prior_artifacts(
    ranking_snapshot: ft.Path, tmp_path: ft.Path, operation: str, location: str
):
    model_dir = tmp_path / "model"
...
@pytest.mark.parametrize("operation", ["train", "evaluate"])
def test_write_failure_never_publishes_partial_artifacts(
    ranking_snapshot: ft.Path, tmp_path: ft.Path, monkeypatch: ft.MonkeyPatch, operation: str
):
    ...
    monkeypatch.setattr("buy_today.ranking.snapshots.write_json", failed_write)
...
@pytest.mark.parametrize(
    "ranker", [RandomRanker(), SVDRanker(n_components=2)], ids=["random", "svd"]
)
def test_generated_cumulative_snapshots_train_and_evaluate_independently(
    ...
    first = generate_dataset(
        write_dataset(working_frame, "zero.csv"),
        distance,
        tmp_path / "step0",
        temperature=86400,
        n_users=3,
    ...
    second = generate_dataset(
        write_dataset(new_batch, "one.csv"),
        distance,
        tmp_path / "step1",
